...
class Tree:
  ...
  def classify(self, classificationData):
    ...
    #YOUR CODE HERE
    node = self.root
    # traverse tree until finding a leafNode
...
def getPertinentExamples(examples,attrName,attrValue):
    ...
    #YOUR CODE HERE
    for d in examples:
        if d.get(attrName) == attrValue: newExamples.append(d)
    return newExamples
...
def getClassCounts(examples,className):
    ...
    for d in examples:
        if d.has_key(className):
            key = d.get(className)  # class Value becomes key of new dict
            if classCounts.has_key(key): classCounts[key] += 1
            else: classCounts[key] = 1
...
def getAttributeCounts(examples,attrName,attrValues,className):
    ...
    for key in attrValues:
        attributeCounts[key] = getClassCounts(getPertinentExamples(examples,attrName,key), className)
...
def setEntropy(classCounts):
    ...
    entropy = 0
    # loop thru counts of each possible class value
    for n in classCounts:
        entropy -= (n / float(total)) * log((n / float(total)), 2)
    return entropy
...
def remainder(examples,attrName,attrValues,className):
    ...
    rem = 0
    for v in attrValues:
        count = 0  # count of examples that have value v for attrName
        # calc count
        for dict in examples:
            if dict.get(attrName) == v: count += 1
        attrEx = getPertinentExamples(examples,attrName,v)
        rem += (count / float(len(examples))) * (setEntropy(getClassCounts(attrEx,className).values()))
    return rem
...
def giniIndex(classCounts):
    ...
    gini = 1
    for n in classCounts:
        gini -= ( n/float(sum(classCounts)) ) ** 2
    return gini
...
def giniGain(examples,attrName,attrValues,className):
    ...
    gain = 0
    for v in attrValues:
        count = 0  # how many examples have value v for attrName
        # calc count
        for dict in examples:
            if dict.get(attrName) == v: count += 1
        attrEx = getPertinentExamples(examples,attrName,v)
        gain += (count / float(len(examples))) * giniIndex(getClassCounts(attrEx,className).values())
    # return inverse of gain if gain is not 0
...
def makeSubtrees(remainingAttributes,examples,attributeValues,className,defaultLabel,setScoreFunc,gainFunc):
    ...
    """
    #YOUR CODE HERE
    if not examples: return LeafNode(defaultLabel)
    ...
    elif not remainingAttributes: return LeafNode(getMostCommonClass(examples,className))
    else:
        # choose best attr based on gainFunc
        maxGain = -1
        for a in remainingAttributes:
            gain = gainFunc(examples,a,attributeValues[a],className)
            if gain > maxGain:
                maxGain = gain
                best = a # update best attr
        node = Node(best)
        for v in attributeValues[best]:
            # remove best(and corresponding values) from args of makeSubtrees
            newRemAttr = list(remainingAttributes)
            newRemAttr.remove(best)
            newExs = getPertinentExamples(examples,best,v)
            newAttrVals = dict(attributeValues)
            newAttrVals.pop(best)
            # build children(subtrees) using updated args
            node.children[v] = makeSubtrees(newRemAttr,newExs,newAttrVals,className,
                                            getMostCommonClass(examples,className),setScoreFunc,gainFunc)
        return node
